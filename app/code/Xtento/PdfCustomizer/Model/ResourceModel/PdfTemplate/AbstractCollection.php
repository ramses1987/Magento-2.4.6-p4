 ...
 * File:          app/code/Xtento/PdfCustomizer/Model/ResourceModel/PdfTemplate/AbstractCollection.php
...
abstract class AbstractCollection extends MagentoAbstractCollection
{
    ...
    public function performAfterLoad($tableName, $linkField)
    {
        ...
        if (!empty($linkedIds)) {
            ...
            $select = $connection->select()->from(['xtento_pdf_store' => $this->getTable($tableName)])
                ->where('xtento_pdf_store.' . $linkField . ' IN (?)', $linkedIds);
