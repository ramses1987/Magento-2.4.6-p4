 ...
 * Last Modified: 2023-06-15T18:36:17+00:00
...
class Items extends AbstractHelper
{
    ...
    public function variableItemProcessor($source, $itemObject, $template)
    {
        ...
        if ($getAllVariables
            ...
            || strstr($templateHtml, 'child_order_item_product_if.') !== false) {
            ...
            if (isset($tempItem)) {
                ...
                if ($getAllVariables || strstr($templateHtml, 'child_order_item_product.') !== false || strstr($templateHtml, ' child_order_item_product_if.') !== false) {
                    ...
                    if (strstr($templateHtml, ' child_order_item_product_if.') !== false) {
                        $transport['child_order_item_product_if'] = $this->formatted->getZeroFormatted($data);
                    }
                }
            }
        }
        ...
        if ($getAllVariables
            ...
            || strstr($templateHtml, 'parent_order_item_product_if.') !== false) {
            ...
            if ($parentItem) {
                ...
                if ($getAllVariables || strstr($templateHtml, 'parent_order_item_product.') !== false || strstr($templateHtml, ' parent_order_item_product_if.') !== false) {
                    ...
                    if (strstr($templateHtml, ' parent_order_item_product_if.') !== false) {
                        $transport['parent_order_item_product_if'] = $this->formatted->getZeroFormatted($data);
                    }
                }
            }
        }
    ...
    public function processItems($source, $templateModel)
    {
        ...
        if (isset($itemConfiguration['sort_by']) && isset($itemConfiguration['sort_order'])) {
            ...
            foreach ($items as $item) {
                try {
                    $product = $this->productRepository->getById($item->getProductId(), false, $source->getStoreId());
            ...
            uasort($items, function($a, $b) use ($sortOrder) {
                if ($sortOrder == 'DESC') {
                    return strnatcmp((string)$b->getXtentoSortField(), (string)$a->getXtentoSortField());
                } else {
                    return strnatcmp((string)$a->getXtentoSortField(), (string)$b->getXtentoSortField());
                }
            });
        }
