...
//        return (in_array([$item['type']], $linkType)) ? true : false;
    ...
    public function getItemHtml($item, $isFirstColumn = '', $idActive = '')
    {
        ...
        if ($item['type'] == '4' && $item['data_type'] && $item['show_title_category'] == "1") {
            ...
            /*if($title=='Top Brands' && isset($_GET['check']) && $_GET['check']==1){
                echo '___'.$title.'___<br>';
                $title = trim($title);
                $title = __($title)."123";
                echo $title;
                die;
        ...
        $icon_title = ($this->hasIcon($item)) ? '<span class="icon_items_sub"><img src=' . $this->filterImage($item) . ' alt="icon items sub" /></span><span class="' . $prefix . 'icon">' . __($title) . '</span>' : __($title);
