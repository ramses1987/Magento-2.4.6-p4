...
namespace Sm\BundleImage\Block\Catalog\Product\View\Type\Bundle;
...
class Option extends \Magento\Bundle\Block\Catalog\Product\Price
{
    ...
    public function __construct(
        ...
        \Sm\BundleImage\Helper\Data $bundleImageHelper,
        array $data = []
    )
    {
        $this->pricingHelper      = $pricingHelper;
        $this->_bundleImageHelper = $bundleImageHelper;
    ...
    public function getSelectionQtyTitlePrice($selection, $includeContainer = true)
    {
        ...
        $imgHtml = '<img class="thumb-bundle" src="' . $this->getProductImage($selection) . '" />';

        $priceTitle = $imgHtml . '<span class="product-name">'
    ...
    public function getSelectionTitlePrice($selection, $includeContainer = true)
    {
        ...
        $priceTitle = $imgHtml . '<span class="product-name">' . $this->escapeHtml($selection->getName()) . '</span>';
        $priceTitle .= ' &nbsp; ' . ($includeContainer ? '<span class="price-notice">' : '') . '+'
            . $this->renderPriceString($selection, $includeContainer) . ($includeContainer ? '</span>' : '');
        return $priceTitle;
    }

    public function getProductImage($selection)
    {
        $imageDisplayArea = 'sm_bundle_option_small';
        $imgUrl           = $this->_bundleImageHelper->getItemImage($selection->getId(), $imageDisplayArea);

        return $imgUrl;
    }

    public function getProductLink($selection)
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $_product      = $objectManager->get('Magento\Catalog\Model\Product')->load($selection->getId());
